	...
	return {
		...
		preload: function (name) {
			...
			return function () {
				...
				IO.evt.progress.map(function (p) {
...
(function (IO) {
	...
	IO.timer = function (time, cb, done) {
...
(function (IO) {
	...
	colors = {
		...
		get: function (idx) {
			return palette[IO.utils.clamp(idx % palette.length, 0, palette.length - 1)];
		},
...
(function (IO) {
	...
	input = {
		...
		init: function (dom, icade) {
			...
			icade = icade || IO.urlParams.icade;
...
(function (IO) {
	...
	var Image = IO.Class.extend({
		...
		init: function (path, flipFlags, scale) {
			...
			IO.gfx.loadImage(path, function (img){
...
(function (IO) {
    ...
    var Tiled = IO.Class.extend({
        ...
        init: function (file, onload) {
            ...
            IO.utils.ajax(file, function (xhr) {
        ...
        layerByName: function (name) {
            ...
            var layer = IO.utils.getByKeyValue(this.layers, "name", name);
            return layer ? [layer] : [];
...
(function (IO) {
	...
	var TrackingCamera = IO.Camera.extend({
		...
		tick: function () {
			...
			var center = IO.utils.center(this, this.zoom),
				e = this.entity,
				xr = this.xRange,
				yr = this.yRange;
		...
		render: function (gfx, renderables) {
			...
			this._super(gfx, renderables.concat([{
				render: function (gfx, cam) {
					...
					var center = IO.utils.center(cam, cam.zoom);
	...
	IO.TrackingCamera = TrackingCamera;
...
(function (IO) {
	...
	var Dialog = IO.Class.extend({
		...
		tick: function (delta) {
			...
			if (this.killKey && IO.input.pressed(this.killKey)) {
				IO.input.release(this.killKey);
				this.done();
			}
...
(function (IO) {
	...
	*/
	...
	var Gravity = IO.Trait.extend({
		...
		tick: function (t) {
			...
			if (this.falling) {
				t.accY += 0.25;
				t.accY = IO.utils.clamp(t.accY, 0, 20);
			} else {
				t.accY = 0;
			}
	...
	*/
	var RemoveAfter = IO.Trait.extend({
	...
	*/
	var Sin = IO.Trait.extend({
		...
		tick: function (t) {
			...
			this[t.target] += Math.sin(IO.utils.now() / t.speed) * (t.amp / 10);
...
(function (IO) {
	...
	var SpriteSheet = IO.Class.extend({
...
(function (IO) {
    ...
    var SpriteAtlas = IO.Class.extend({
        ...
        initPList: function (plist) {
            ...
            var resolve = IO.preload(plist);
            IO.utils.ajax(plist, function (xhr) {
        ...
        initCSV: function (path) {
            ...
            var resolve = IO.preload(path);
            IO.utils.ajax(path + ".txt", function (xhr) {
                IO.gfx.loadImage(path + ".png", function (img) {
        ...
        loadImages: function (imageData) {
            ...
            imageData.forEach(function (imgData) {
                IO.gfx.loadImage(self.path + imgData.path, function (img) {
...
(function (IO) {
	...
	var Anim = IO.Class.extend({
		...
		tick: function () {
			...
			var diff = IO.utils.now() - this.frameTime;
			this.changed = false;
			this.rewound = false;

			if (diff > this.speed) {
				this.frameTime = IO.utils.now() + (Math.min(this.speed, diff - this.speed));
...
(function (IO) {
	...
	var Map = IO.Class.extend({
		...
		getBlockEdge: function(pos, vertical) {
		    ...
		    return IO.utils.snap(pos, snapTo);
		...
		*/
		imgToCells: function (img, colourMap, cb, flipFlags) {
			...
			if (typeof img === "string") {
				// Load first
				IO.gfx.loadImage(img, function (canvas){
...
(function (IO) {
	...
	var DebugMap = IO.Map.extend({

		init: function (tileW, tileH, xTiles, yTiles, cells, walkable, seed) {

			var ctx = this.ctx = IO.gfx.createCanvas(tileW * xTiles, tileH * yTiles),
				data = IO.gfx.ctx.createImageData(tileW * xTiles,tileH * yTiles),
				pix = data.data,
			...
			oldSeed = IO.utils.rnd.seed;
			IO.utils.rnd.seed = this.seed;
			off = IO.utils.rnd.rand(255);

			for (var i = 0; i < numPix; i++) {
				var row = i / data.width | 0,
					col = ((i / tileW) | 0) % data.width % xTiles,
					noise = IO.utils.rnd.rand(100) < 30 ? (IO.utils.rnd.rand(30)) : 0,
			...
			IO.utils.rnd.seed = oldSeed;

			ctx.putImageData(data, 0, 0);

			this._super(
				new IO.SpriteSheet(ctx.canvas, tileW, tileH),
				cells,
				walkable);
...
(function (IO) {
	...
	RayCastMap = IO.Map.extend({
		...
		setStripWidth: function (stripWidth) {
			...
			this.numRays = Math.ceil(IO.env.w / stripWidth);
		...
		setFOV: function (fov) {
			...
			var fovRad = IO.utils.deg2rad(fov);
			this.viewDistance = (IO.env.w / 2) / Math.tan((fovRad / 2));
		...
		castRays: function (entities, p) {
			...
			for (i = 0; i < this.numRays; i++) {
				...
				hit = IO.rays.cast(p.rotation + rayAngle, p.x + p.w / 2, p.y + p.h / 2, this, entities);
...
(function (IO) {
	...
	var Game = IO.Class.extend({
		...
		init: function (w, h) {
			...
			IO.env.w = ctx.canvas.width;
			IO.env.h = ctx.canvas.height;

			IO.gfx.init(ctx);
			IO.input.init(ctx.canvas);

			IO.evt.onload(function () {
				self.load();
				self.run(Date.now());
			});
			...
			IO.utils.now = function () {
				return self.now();
			};

			this.stats = IO.utils.Stats();
		...
		setScreen: function (screen, opts) {
			...
			if (this.screenPrev) {
				...
				IO.timer(opts.time || 20, function (ratio) {
