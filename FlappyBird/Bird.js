(function (IO) {
    ...
    var Bird = IO.Entity.extend({
        ...
        font: new IO.Font("res/flapfont.png", 16, 22, "abcdefghijklmnopqrstuvwxyz"),
        font2: new IO.Font("res/flapfont2.png", 16, 21, "abcdefghijklmnopqrstuvwxyz"),

        sounds: {
            "hit": new IO.Sound("res/audio/sfx_hit", 1)
        },
        ...
        tick: function () {
            ...
            switch (this.state.get()) {
                ...
                case "RUNNING":
                    ...
                    if (IO.input.pressed("jump")) {
                       //this.ac = this.jumpAc;
                    }

                    this.handleKeys();

                    if (this.y > IO.env.h - 112 - this.h) {
                        this.y = oldy;
                        this.die();
                    }
                    break;
        ...
        handleKeys: function () {
            var rightKey = false;
            if (IO.input.lastKey) {
                if (String.fromCharCode(IO.input.lastKey).toLowerCase() === this.curWord[this.curIdx]){
                    this.ac = -7;
                    this.curIdx++;
        ...
        render: function (gfx) {
            ...
            window.game.atlas.render(
                gfx,
                "bird" + this.color + "_" + IO.utils.toggle(this.flapping, 3),
                this.x - 11,
                this.y - 17);
